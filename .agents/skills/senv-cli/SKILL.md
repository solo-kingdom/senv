...
- KeyPair Tab：独立 Tab（`mgr.SSH` 非空时紧随 SSH Tab），两栏（分组侧栏 / keypair 列表），组语义与 host 一致（All 置顶 → 字母序 → 「未分组」置底）。列表栏 `n`/`i` 导入（表单含 name/private key file/group）、`r` 重命名（自动联动 host `identityKey`）、`e` 编辑 group（仅 group 字段的小表单）、`d` 删除、`A` 落盘（与 `senv keypair export` 同一路径）、`D` 设/取消本机默认（写 `groups/_default.conf` 的 `Host *`）、`p` prune（两栏均可用；异步列出未被任何 host 引用的落盘私钥，当前默认钥跳过；vault 中仍有对应 keypair 的标注 `(keypair still in vault)`；`enter`/`y` 删除与 `senv keypair prune` 同一白名单，`esc`/`n` 取消零删除；vault 档案永不触碰），`enter` 详情（公钥无缩进、展示 OpenSSH comment/邮箱与是否默认），`v` 按需预览私钥（详情弹层，关闭即丢弃；列表永不渲染私钥），`/` 按名称过滤；行内 `被 N 个 Host 引用`、`default` 标记、零引用灰显「未被引用」。被引用 keypair 默认拒绝删除并列出引用者，按 `F` 才强制删除并清空 host `identityKey`；export 确认后只提示落盘路径。
...
- `senv keypair edit <name> --group <g>` 免编辑器单字段改分组（group 是唯一可就地编辑的 keypair 元数据，key 材料不动；空值 = 未分组；组名禁止含 `/`）。改组只影响未来 export 路径，已落盘文件不迁移（用 `keypair prune` 清理旧路径残留）；若该钥是本机默认，会改写 `_default.conf` 的 IdentityFile。未显式给 `--group` 时报参数错误，不进编辑器。
- `senv keypair export <name>`（`materialize` 为过渡别名）把 private key 明文写到 `~/.ssh/senv/keys/<分组>/<名>`（未分组入 `_ungrouped`；目录 0700、文件 0600），与 `host export` 应用模式的落盘路径相同。仅在用户明确要求时使用；删除 vault 记录不会自动删除已落盘文件（用 `keypair prune` 清理未引用文件）。
