...
- LLM Tab：两栏（provider / agent）。provider 栏 `n` 新建、`e` 编辑（别名只读）、`r` 重命名（联动自有凭据与本机 agent 指针；不改写 coding agent 原生配置，需重跑 `senv ai switch`）、`d` 删除、`enter` 详情、`/` 过滤（匹配 alias）、`R` 联网刷新公开模型目录缓存（与 `senv ai refresh` 同语义，失败保留旧缓存；`Ctrl+R` 仍是本地档案重载、不访问网络）；agent 栏 `↑↓` 选择、`s` 以左栏选中 provider 切换——**焦点在左栏时先弹 agent 多选**（`space` 勾选、`a` 全选/取消、`esc` 取消；进入时勾选真实状态即已指向该 provider 的 agent；空集不可提交），焦点在右栏时直接作用于右栏光标 agent；随后 `space` 多选 Agent 模型集（进入默认全选）→ 选定默认模型 → 确认；选中多个 agent 时逐个写回并汇总结果，单个失败不中止其余（失败在提示条列出）。`M` 对已指向的 agent 仅换默认模型（与 `s` 成对，大写为变体；**不弹 agent 多选**），候选限定在该 agent 已写入的模型集内（未指向时提示先按 `s`）。agent 行展示 `provider / 默认模型（N 个模型）`，指针模型已不在档案中时附 `⚠` 漂移标记。provider 表单覆盖 base_url、`api_shape`、三个形态地址（chat / responses / anthropic URL；anthropic 填 root，不带 `/v1`）、目录来源、模型集、模型上下文、模型输出、模型推理、默认推理档、输入模态、默认模型与凭据来源；模型上下文用 `<model>=<tokens>` 逗号分隔，默认推理档用 `<model>=<effort>` 或集合级单一档位，输入模态用 `<model>=<mod>[,<mod>...]`。凭据默认从既有 env/text 条目中选择，也可选「新建自有凭据」用遮蔽输入写入 `text:llm-keys/<alias>`，明文不进 TUI 状态或渲染文本。枚举/引用字段聚焦时下方列出候选值。详情弹层展示接入形态、三个形态地址与各模型已保存的 context / output / reasoning / 默认推理档 / 输入模态。
...
- 档案与凭据存 vault：`senv ai provider add/edit/rename/list/show/remove`。`show`/`list` 不返回凭据明文；`add`/`edit` 可带 `--description`（档案说明，不是模型目录文案）；`add` 禁止 `--api-key`，用 TTY prompt、`--api-key-stdin` 或 `--key-ref env:<group>/<key>`。HTTP base URL 必须显式 `--allow-http`。MCP `llm_provider_list` 只读返回档案元数据（含 `api_shape` 与已设置的 `shape_urls`，未声明则省略），不含凭据明文。
