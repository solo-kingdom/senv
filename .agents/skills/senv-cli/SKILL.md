...
- Agent 模型集按各 agent 原生机制落盘，使 agent 自己的模型选择器能在集合内换模型：claude-code 写 `modelPicker`（`replaceBuiltInOptions: true`，替换内置 lineup）、codex 生成 `~/.codex/model-catalogs/senv-<alias>.json` 并让 `model_catalog_json` 指向它、kimi 每个模型一条 `[models."senv-<alias>/<m>"]`、pi 写 `providers.<id>.models[]`、opencode 写 `provider.<id>.models{}`。模型集超过 20 个时命令提示可用 `--models` 缩小。
