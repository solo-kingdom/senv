...
- `senv keypair edit <name> --group <g>` 免编辑器单字段改分组（group 是唯一可就地编辑的 keypair 元数据，key 材料不动；空值 = 未分组；组名禁止含 `/`）。改组只影响未来 materialize/export 路径，已落盘文件不迁移（用 `keypair prune` 清理旧路径残留）。未显式给 `--group` 时报参数错误，不进编辑器。
