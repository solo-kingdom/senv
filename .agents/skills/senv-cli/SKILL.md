...
- 全局：`Tab`/`Shift+Tab` 循环；`1`–`9` 按注册顺序直达（越界忽略；顺序 Env、Text、Config、SSH、KeyPair、LLM、MCP、History、Audit、Backup——Backup 恒在末位，全部注册时它是第 10 个、无数字键，用 `Tab` 到达）；`Ctrl+R` 刷新当前 Tab；`S` 跨类型搜索（只匹配标识：key/name、host alias/hostname/group/tags、provider alias、MCP 档案 alias/command、backup 的 group/key/description，不匹配值/私钥/凭据/MCP env 值/backup 正文）；`?` 键位总览（与实际键位同源，不会漂移）；`esc` 回上一层（清过滤/关弹层/向导回退）；`q` 退出（仍有待推送时先提示一次）。列表 Tab 通用导航：`↑↓/jk`、`←→/hl` 切栏、`g`/`G` 跳顶底、`PgUp/PgDn` 翻页。
...
- LLM Tab：两栏（provider / agent）。provider 栏 `n` 新建、`e` 编辑（别名只读）、`r` 重命名（联动自有凭据与本机 agent 指针；不改写 coding agent 原生配置，需重跑 `senv ai switch`）、`d` 删除、`enter` 详情、`/` 过滤（匹配 alias）、`R` 联网刷新公开模型目录缓存（与 `senv ai refresh` 同语义，失败保留旧缓存；`Ctrl+R` 仍是本地档案重载、不访问网络）；agent 栏 `↑↓` 选择、`s` 以左栏选中 provider 切换——**焦点在左栏时先弹 agent 多选**（`space` 勾选、`a` 全选/取消、`esc` 取消；进入时勾选真实状态即已指向该 provider 的 agent；空集不可提交），焦点在右栏时直接作用于右栏光标 agent；随后 `space` 多选 Agent 模型集（进入默认全选）→ 选定默认模型 →（选中 agent 含 claude-code 的完整切换才有此步）选定后台模型（步骤带说明：写入 `ANTHROPIC_SMALL_FAST_MODEL`；候选为 Provider 模型集全集，初始游标在档案声明值、缺省为刚选的默认模型）→ 确认；选中多个 agent 时逐个写回并汇总结果，单个失败不中止其余（失败在提示条列出）。`M` 对已指向的 agent 仅换默认模型（与 `s` 成对，大写为变体；**不弹 agent 多选**），候选限定在该 agent 已写入的模型集内（未指向时提示先按 `s`）。agent 行展示 `provider / 默认模型（N 个模型）`，指针模型已不在档案中时附 `⚠` 漂移标记。provider 表单覆盖 base_url、`api_shape`、三个形态地址（chat / responses / anthropic URL；anthropic 填 root，不带 `/v1`）、目录来源、模型集、模型上下文、模型输出、模型推理、默认推理档、输入模态、默认模型、后台模型（claude-code 压缩等后台任务用，空 = 切换时回退默认模型）与凭据来源；模型上下文用 `<model>=<tokens>` 逗号分隔，默认推理档用 `<model>=<effort>` 或集合级单一档位，输入模态用 `<model>=<mod>[,<mod>...]`。凭据默认从既有 env/text 条目中选择，也可选「新建自有凭据」用遮蔽输入写入 `text:llm-keys/<alias>`，明文不进 TUI 状态或渲染文本。枚举/引用字段聚焦时下方列出候选值。详情弹层展示接入形态、三个形态地址、默认/后台模型与各模型已保存的 context / output / reasoning / 默认推理档 / 输入模态。
- MCP Tab：两栏（档案 / 导出目标 agent，agent 集合与 `senv mcp export` 相同，含 claude-desktop/cursor）。档案栏 `n` 新建、`e` 编辑（别名只读）、`d` 删除（不自动撤回）、`i` 导入（路径表单，`~` 展开；与 `senv mcp import` 同一解析/传输识别/冲突跳过语义，完成后弹出结果报告：逐条 create/conflict/failed 及原因 + 计数，不含值；TUI 不提供 `--dry-run`）、`enter` 详情、`/` 过滤（匹配 alias/command）；`s` 在 user/project 间切换导出 scope（会话内有效、不持久化，默认 user；目前仅 cursor 区分，project 写 CWD 相对的 `.cursor/mcp.json`，其余 agent 两 scope 同路径）；`x`/`u` 当前档案 × 当前 agent，`X`/`U` 当前档案 × 全部 agent（均按当前 scope）；**焦点在左栏时这四个键先弹 agent 多选**（`space` 勾选、`a` 全选/取消、`esc` 取消；进入时勾选真实状态即该档案已导出到的 agent，`a` 全选等价于原来的 `X`/`U`），焦点在右栏时保持上述原语义。表单含 transport 选择（stdio/http/sse）：stdio 下填 command/args/env，http/sse 下只出现 url/headers（headers 经 `$EDITOR` 按 `Name: Value` 行编辑）。计划页 `y` 确认 / `esc` 取消 / `F` 覆盖漂移；撤回被改过的条目逐条 `y/n`。列表/计划是摘要，不渲染值（remote 列表只显示 `scheme://host` 与 env 键数）；详情弹层完整渲染字段：url 含 query、header 为 `Name: Value`、env 为 `KEY=value`（模板引用原样）。表单预览只显示键名，值仅在 `$EDITOR` 内可见。`mcp install` / `serve` / `list-tools`、`--print` 仍走 CLI。
- History Tab（server 模式注入；首次激活才查询）：recent（vault 最近变更，行含时间/条目标识/版本号/解密预览）→ `enter` 进该条目时间线 → `enter` 打开选中版本的**滚动详情弹层**（完整解密内容，`↑↓`/`PgUp`/`PgDn` 滚动，`esc`/`enter` 关闭并回到时间线）。列表行列宽按面板实际宽度分配（超宽才以 `…` 截断）；`R` 恢复选中版本（确认后走既有推送流程，产生新 revision），`esc` 从时间线退回 recent。
- 只读详情：Config/SSH/KeyPair/LLM/MCP/History 列表按 `enter` 打开详情弹层（长 `base_url`、模型列表、路径在列表里截断显示；AI/KeyPair/SSH/MCP/History 的弹层可滚动看全文）。
...
- 档案与凭据存 vault：`senv ai provider add/edit/rename/list/show/remove`。`show`/`list` 不返回凭据明文；`add`/`edit` 可带 `--description`（档案说明，不是模型目录文案）；`add` 禁止 `--api-key`，用 TTY prompt、`--api-key-stdin` 或 `--key-ref env:<group>/<key>`。HTTP base URL 必须显式 `--allow-http`。`add`/`edit` 可带 `--background-model <model>` 声明档案后台模型（必须属于模型集；edit 传空串清除声明，未传则保留）。MCP `llm_provider_list` 只读返回档案元数据（含 `api_shape`、`background_model` 与已设置的 `shape_urls`，未声明则省略），不含凭据明文。
...
- `senv ai provider rename <old> <new>`：同次操作内改档案别名；规范自有凭据 `text:llm-keys/<old>` 一并改名并更新 `credential_ref`；**env 全部分组中值精确等于 `{{text:llm-keys/<old>}}` 的条目改写为 `{{text:llm-keys/<new>}}`**（如 codex switch 写入的 SeedRef）；本机 agent 指针中的 provider 字段同步改写。目标别名或目标 `llm-keys/<new>` 已存在时拒绝且零写入。外部 `--key-ref` 不移动。不改写 coding agent 原生配置或 catalog——输出提醒重跑 `senv ai switch <agent> <new>`。TUI LLM Tab `r` 等价。
- 接入地址统一按 OpenAI 兼容形态落库：`add`/`edit` 会补末段 `/v1` 并收敛尾斜杠，改写时提示；已归一（末段为 `/v1` 或纯数字版本段如 `/v4`，智谱 open.bigmodel.cn 即此类）的输入静默通过；`/v1beta` 等带字母后缀的变体仍不识别、会补成 `…/v1beta/v1`。`list`/`show` 展示的是落库值。
- 形态地址（per-shape URLs，ADR-0027）：重复 flag `--shape-url <api_shape>=<url>`（add/edit，key 取 `openai-chat`/`openai-responses`/`anthropic`）为特定接口形态声明独立接入地址，适合三端点不共根的网关。`openai-chat`/`openai-responses` 的值沿用 base URL 的兼容归一；`anthropic` 的值**原样存储、仅收敛尾斜杠**——应填 claude-code 将拼 `+ /v1/messages` 的 root（如 `…/api/anthropic`），传带 `/v1` 的值不会被修正。edit 传值设置、空值（`--shape-url anthropic=`）清空、省略的 key 保留；flag 给了但无有效值清空全部。URL 校验与 `--allow-http` 门禁与 base URL 一致；`list`/`show` 展示三个形态地址（未设 `-`）。
- `--api-shape`（`openai-chat` | `openai-responses` | `anthropic`）可选声明接口形态；`--api-shape ""` 清除回推断。留空时 `switch` 按目标 agent 协议族归一接入地址；声明后成为兼容判据，但**目标协议族存在显式形态地址时放行**（一份档案可同时服务多族）；无显式地址且与目标 agent 协议族不匹配时 `switch` 拒绝写文件，错误给出三个动作：改档案形态 / 补配该族 `--shape-url` / 换 provider。形态地址的存在性**不反向决定** OpenAI 族内的线协议。
- `senv ai switch <claude-code|codex|kimi|pi|opencode> <provider> [--models m1,m2] [--default-model D]` 会事务式改写目标 coding agent 的原生配置并保存本机指向。**省略 `--models` 即全选 Provider 模型集**，显式给出时保序（逗号分隔或重复给出均可，去重保序）；`--default-model` 只覆盖本次写入的起始模型，**不回写档案**。`--background-model` 同理覆盖本次的后台模型（仅 claude-code 消费）：后台模型按「`--background-model` > 档案 `background_model` > 默认模型」解析并写入 `~/.claude/settings.json` 的 `env.ANTHROPIC_SMALL_FAST_MODEL` 与同值的 `env.ANTHROPIC_DEFAULT_HAIKU_MODEL`（前者管会话压缩等后台任务，后者把 haiku 档位整体重映射；这两个键由 senv 管理，切换覆盖手写值）——缺省必须回退默认模型，因为 claude-code 内置缺省 `claude-haiku` 在自建网关上不存在、会让自动压缩静默失败；走回退时 stderr 给 warning，提示用 `senv ai provider edit <alias> --background-model <model>` 声明更便宜的模型。显式值与档案声明都必须属于 Provider 模型集，否则切换拒绝写文件。`CLAUDE_CODE_AUTO_COMPACT_WINDOW` 等其余 env 键不归 senv 管理（压缩阈值走 modelPicker `behavesAs` 的 per-model 投影，档案 `context_window` 写错就 `provider edit` 修数据）。`--model` 已移除：出现即以非 0 退出并提示替代用法（参数校验发生在解锁与写盘之前）。接入地址按目标族解析：显式形态地址优先——claude-code 用 `anthropic_base_url` 原样写回，OpenAI 族按已解析线协议取 `chat_base_url`/`responses_base_url`；未设回落 `BaseURL` 按协议族转换（claude-code（Anthropic Messages）剥离末段 `/v1`，其余保持带版本形态）；命令输出 provider、默认模型、模型集条数、实际写入的接入地址及其来源（显式字段名 / 由 BaseURL 推断）。切换后多数 agent 配置中会出现解密后的 API key（文件 0600）；**Codex 只写 `env_key` 名，明文不落盘**，且该名字保证可由 `senv env export` 提供——`credential_ref` 为 `env:<g>/<k>` 时 `env_key` 就是 `<k>`（shell 里既有的名字，零额外操作）；为 `text:<g>/<k>`（含自有凭据 `text:llm-keys/<alias>`）时是 `SENV_<ALIAS>_API_KEY`（alias 净化大写、`-` 变 `_`），切换会在**默认 env 组**补一条同名引用条目（值 `{{text:<g>/<k>}}`，已存在同名条目则不覆盖）。因此 codex 取凭据的前提是 `eval "$(senv env export --if-session)"` 已生效：引用组未激活时切换会给出可操作 warning（`senv env group activate <g>`），同名条目被占用且值不同时也会提示核对；已运行的 `codex app-server`（Codex Desktop / SSH 远程）只在启动时拷贝环境，export 后需 `pkill -f 'codex app-server'` 再重开，否则 TUI 仍报 `Missing environment variable`（同机 `codex exec` 却能通）。档案跨机同步后 `credential_ref` 指向的条目可能尚未在本机：`switch` 保持 fail-closed（零写入，codex 同样生效），错误指明缺失的完整引用（如 `text:llm-keys/<alias>`）与修复指引（`senv text add` 或从已有机器同步）。
...
- 档案显式声明的 `api_shape` 决定 kimi/pi/opencode 在 OpenAI 兼容族内的线协议：pi `api` 字段与 kimi provider `type` 在 `openai-completions`/`openai`（chat）与 `openai-responses`/`openai_responses`（responses）间选择，opencode 在声明 `openai-responses` 时用 `@ai-sdk/openai`；未声明时维持各自既有默认。codex 上游已移除 chat 线协议，`wire_api` 恒写 `responses`、地址取 `responses_base_url`（未设则回落 `BaseURL`）；声明 `openai-chat` 且无 `responses_base_url` 的档案切换 codex 会被拒绝。实际写入地址跟随已解析线协议：chat 时取 `chat_base_url`，responses 时取 `responses_base_url`，对应形态地址未设则回落 `BaseURL`。
...
- `senv ai status` 免解锁可用，显示 `provider / 默认模型（N 个模型）` 与切换时间；vault 已解锁且指针里的模型已不在档案中（档案缩集/改名）时附漂移提示，判定只看指针与档案、**不解析 agent 配置文件**，档案不可得时省略提示。TUI LLM Tab 的 `M` 是「仅换默认模型」入口（限定在已写入的 Agent 模型集内）。
...
- 导出：`senv mcp export --agent codex,cursor` 或 `--all`（必须显式给目标，没有默认全量）。按目标 agent 的格式合并写入其**全局配置**：JSON 族 stdio 写 `command/args/env`、remote 写 `type/url/headers`；Codex TOML 写 `url`（+`transport`）与 `http_headers = { Name = "Value" }`（不是 JSON 族的 `headers`，已在 codex-cli 0.153.4 上验证：`codex mcp get` 解析并生效）。`--dry-run` 只出计划，`--print` 只输出片段，二者都不落盘。
- **remote 能力矩阵**：目标 agent 配置格式表达不了的条目在计划里标 `error` 并说明原因，该 agent 文件不动、其余 agent 继续（已知：claude-desktop 不支持任何 remote 条目；codex remote 通过 `http_headers` 支持 headers；zcode/kimi 未核验 sse）。senv 只写各 agent 文档化键，不猜键名：`type`/`transport` 只在声明的目标上写（pi、kimi 无该键，http 与 sse 落盘形状相同）。**pi 无内置 MCP**：配置写 `$PI_CODING_AGENT_DIR/mcp.json`（默认 `~/.pi/agent/mcp.json`），由 pi-mcp-adapter 扩展读取（`pi install npm:pi-mcp-adapter`，stdio/http/sse + headers 均支持）；此类适配器目标的前置依赖会随安装/导出计划输出，并在写盘前自动尝试安装（检测 `settings.json` 的 `packages` 已装则跳过；安装失败/不在 PATH 只提示，导出与写盘继续，失败不计入导出失败）。
...
- senv-server 管理面（独立二进制，不经 cobra、不在 `senv --help`；`--dsn` 缺省取环境变量 `SENV_SERVER_DSN`，任何能连到 PG 的主机都可执行）：
  ...
  - `serve` `--alert-body-template`（env `SENV_SERVER_ALERT_BODY_TEMPLATE`）：把告警 body 渲染成 provider 专有格式（飞书机器人需 `{"msg_type":"text","content":{"text":"…"}}` 且文本含群关键词）。变量 `{{.alert}}` `{{.time}}` `{{.ip}}` `{{.user_id}}` `{{.client_id}}` `{{.user}}` `{{.client}}` `{{.reason}}` `{{.count}}`；不配置则投默认扁平 JSON。注意 **HTTP 200 ≠ 送达**：飞书一类网关拒收时返回 200 + `{"code":19024,…}`，server 会读响应 envelope 里的 `code`/`errcode`/`error_code` 判失败并重试，最终丢弃的日志带状态码与响应体片段。
...
senv ai provider edit <alias> [flags]    # 就地编辑档案（别名不可改；--api-shape 声明/清除形态；--shape-url <shape>=<url> 配形态地址）
