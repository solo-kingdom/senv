...
Coding Agent 后台任务（如 claude-code 的会话压缩）使用的模型，区别于用户面对的主模型；LLM Provider 档案可显式声明它（必须属于该档案模型集），切换时按「单次切换指定 > 档案声明值 > 默认模型」解析并写入 agent 配置。它必须是对应 Provider 实际可用的模型：不可用时后台任务静默失败、上下文无限膨胀。
_Avoid_: 小模型（泛指时）、压缩模型（只覆盖压缩场景时）
