...
2026-09-13 修订：MCP 详情弹层改为完整渲染字段值（url 含 query、header 值、env 字面量/模板原样），上段「详情不出现字面量、渲染文本不得含凭据明文」对详情不再成立——详情是显式的查看动作，用户接受其截屏暴露面。列表/计划的摘要不渲染原则、表单预览只显示键名、值仅在 `$EDITOR` 内可见，均维持不变。
