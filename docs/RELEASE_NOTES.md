...
- Session caches are created only on an operating-system-verified memory-backed filesystem (Linux: tmpfs/ramfs). Disk-backed or unknown `XDG_RUNTIME_DIR` and fallback filesystems now fail closed for every timeout, including `never`; one-time interactive password use remains available.
