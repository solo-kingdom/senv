...
// backupTab renders Backup data: left column = groups, right column = backup block
...
type backupTab struct {
	...
	itemsByGroup map[string][]backupItemRow
	groupIndex   int
	loaded       bool
...
const (
	backupModeNormal backupMode = iota
	backupModeDeleteConfirm
	backupModeExportPath
	backupModeNewKey
	backupModeAddGroup
	backupModeFilter
	backupModeBatchDeleteConfirm
	backupModeBatchExportPath
	backupModeDeleteGroupConfirm
)

func newBackupTab(mgr Managers) *backupTab {
	ti := textinput.New()
	ti.CharLimit = 0
	return &backupTab{mgr: mgr, focusLeft: true, input: ti}
}

func (t *backupTab) Title() string { return "Backup" }

func (t *backupTab) Bindings() []KeyAction {
	if t.form != nil {
		return formBindings()
	}
	switch t.mode {
	case backupModeFilter:
		return filterBindings(false)
	case backupModeExportPath, backupModeBatchExportPath:
		return []KeyAction{
			{[]string{"enter"}, "export", grpForm, false},
			{[]string{"esc"}, "cancel", grpForm, false},
		}
	case backupModeNewKey:
		return []KeyAction{
			{[]string{"enter"}, "open vim", grpForm, false},
			{[]string{"esc"}, "cancel", grpForm, false},
		}
	case backupModeAddGroup:
		return []KeyAction{
			{[]string{"enter"}, "create", grpForm, false},
			{[]string{"esc"}, "cancel", grpForm, false},
		}
	case backupModeDeleteConfirm, backupModeDeleteGroupConfirm:
		return confirmBindings()
	case backupModeBatchDeleteConfirm:
		return []KeyAction{
...
func (t *backupTab) cursorForFocus() int {
	if t.focusLeft {
		return t.groupIndex
	}
	return t.itemIndex
...
func (t *backupTab) InputMode() bool {
	if t.form != nil {
		return true
	}
	switch t.mode {
	case backupModeFilter, backupModeExportPath, backupModeNewKey, backupModeAddGroup,
		backupModeDeleteGroupConfirm, backupModeBatchDeleteConfirm, backupModeBatchExportPath:
		return true
	}
	return false
}
...
func (t *backupTab) Reload() tea.Cmd {
	// stale-while-revalidate：后台重载期间旧数据保持可见，完成后静默替换。
	return t.load()
}

func (t *backupTab) load() tea.Cmd {
	mgr := t.mgr.Backup
	return func() tea.Msg {
		st := perflog.Start("tui.load-backup")
		if mgr == nil {
			st.End(false)
			return backupLoadedMsg{err: fmt.Errorf("backup manager unavailable")}
		}
		// 单趟快照：分组与全部条目一次批量装载（一次 vault 读锁），并与进程内
		// memo 共享；写操作/pull 应用后由 model 层 Invalidate。
		snap, err := backupSnapshot(t.mgr)
		if err != nil {
			st.End(false)
			return backupLoadedMsg{err: err}
		}
		groups := make([]backupGroupRow, 0, len(snap.Groups))
		itemsByGroup := make(map[string][]backupItemRow, len(snap.Groups))
		totalKeys := 0
		for _, g := range snap.Groups {
			// 侧栏范式：空分组也显示（计数 0），与过滤期行为一致
			groups = append(groups, backupGroupRow{name: g.Name, description: g.Description, keyCount: g.KeyCount})
			itemsByGroup[g.Name] = buildBackupItems(g.Name, snap.Items[g.Name])
		...
		groups = append([]backupGroupRow{{name: backupAllLabel, isAll: true, keyCount: totalKeys}}, groups...)
		st.With("groups", len(groups)).End(true)
		return backupLoadedMsg{groups: groups, itemsByGroup: itemsByGroup}
	}
}

func buildBackupItems(group string, infos []backup.BackupInfo) []backupItemRow {
	out := make([]backupItemRow, 0, len(infos))
	for _, ti := range infos {
		out = append(out, backupItemRow{
			group:       group,
			key:         ti.Key,
...
// backupAllLabel 是 text 侧栏顶部的 All 伪组标签。
const backupAllLabel = "All"

func (t *backupTab) currentGroup() string {
...
func (t *backupTab) currentGroupRow() (backupGroupRow, bool) {
	if t.groupIndex < 0 || t.groupIndex >= len(t.groups) {
		return backupGroupRow{}, false
	}
	return t.groups[t.groupIndex], true
}

// realGroup 返回当前选中的真实分组名；All 伪组返回 ""。
func (t *backupTab) realGroup() string {
	if row, ok := t.currentGroupRow(); ok && row.isAll {
		return ""
	}
...
func (t *backupTab) focusGroup(it backupItemRow) string {
	if it.group != "" {
		return it.group
	}
	return t.currentGroup()
...
func (t *backupTab) filteredItems() []backupItemRow {
	var all []backupItemRow
	...
	out := make([]backupItemRow, 0, len(all))
	for _, it := range all {
...
func (t *backupTab) currentItem() (backupItemRow, bool) {
	items := t.filteredItems()
	if t.itemIndex < 0 || t.itemIndex >= len(items) {
		return backupItemRow{}, false
	}
...
func (t *backupTab) Update(msg tea.Msg) (Tab, tea.Cmd) {
...
func (t *backupTab) listForFocus() []string {
	if t.focusLeft {
...
func (t *backupTab) moveCursor(delta int) {
	if t.focusLeft {
...
func (t *backupTab) jumpCursor(idx int) {
	if t.focusLeft {
...
func (t *backupTab) focusJump(group, key string) {
	t.filterBox.Clear()
	t.mode = backupModeNormal
	for i, g := range t.groups {
		if g.name == group {
			t.groupIndex = i
			break
...
func (t *backupTab) handleModalKey(msg tea.KeyMsg) (Tab, tea.Cmd) {
	if t.mode == backupModeBatchDeleteConfirm {
	...
	if t.mode == backupModeDeleteGroupConfirm {
		name := t.currentGroup()
		t.mode = backupModeNormal
		switch msg.String() {
		case "enter", "y":
	...
	if t.mode == backupModeDeleteConfirm {
		switch msg.String() {
		case "enter", "y":
			it, ok := t.currentItem()
			t.mode = backupModeNormal
...
func (t *backupTab) submitModal() (Tab, tea.Cmd) {
	switch t.mode {
	case backupModeBatchExportPath:
...
func (t *backupTab) enterNewKeyMode() (Tab, tea.Cmd) {
	t.mode = backupModeNewKey
	t.input.SetValue("")
...
func (t *backupTab) enterDeleteConfirm() (Tab, tea.Cmd) {
	if _, ok := t.currentItem(); !ok {
		return t, warnToast("no entry to delete")
	}
	t.mode = backupModeDeleteConfirm
	return t, nil
}

// selectionKey 返回当前文本块的稳定选择标识（group/key）。
func (t *backupTab) selectionKey() string {
...
func (t *backupTab) allItems() []backupItemRow {
	var all []backupItemRow
	for _, rows := range t.itemsByGroup {
		all = append(all, rows...)
	}
...
// 已选项；空集回落游标的判定由调用方负责。
func (t *backupTab) selectedTargets() [][2]string {
	var out [][2]string
...
func (t *backupTab) enterBatchExportPath() (Tab, tea.Cmd) {
	if len(t.selectedTargets()) == 0 {
		return t, warnToast("selection is empty or has no entries in the current group")
	}
	t.mode = backupModeBatchExportPath
	t.input.SetValue("")
...
func (t *backupTab) doBatchDelete(targets [][2]string) tea.Cmd {
	mgr := t.mgr.Backup
	mgrs := t.mgr
	return func() tea.Msg {
		failed := 0
		for _, tgt := range targets {
			if err := mgr.Delete(tgt[0], tgt[1]); err != nil {
				recordAudit(mgrs, session.AuditOpBackup, backupTarget(tgt[0], tgt[1]), false, "batch delete failed")
				failed++
				continue
			}
			recordAudit(mgrs, session.AuditOpBackup, backupTarget(tgt[0], tgt[1]), true, "batch delete")
		}
		if failed > 0 {
			return backupReloadMsg{warn: fmt.Sprintf("batch delete finished, %d failed", failed)}
		}
		return backupReloadMsg{toast: fmt.Sprintf("deleted %d entries", len(targets))}
	}
}

// doBatchExport 逐块导出到 dir/<key>.txt；单条失败不中止其余。目标文件名
// 来自用户数据（key），写入前必须复验路径段（securefs 约束）。
func (t *backupTab) doBatchExport(dir string, targets [][2]string) tea.Cmd {
	mgr := t.mgr.Backup
	return func() tea.Msg {
		failed := 0
		for _, tgt := range targets {
...
func (t *backupTab) enterExportMode() (Tab, tea.Cmd) {
	if _, ok := t.currentItem(); !ok {
		return t, warnToast("no entry to export")
	}
	t.mode = backupModeExportPath
	t.input.SetValue("")
...
func (t *backupTab) enterAddGroupMode() (Tab, tea.Cmd) {
	siblings := make(map[string]bool, len(t.groups))
	for _, g := range t.groups {
		siblings[g.name] = true
	}
	f := newForm("new backup group",
...
func (t *backupTab) openForm(f *form, onSubmit func(values map[string]string) tea.Cmd) {
	f.SetSize(t.width, t.height)
	t.form = f
	t.formSubmit = onSubmit
}

// defaultBackupGroup is the group init/EnsureDefault create; it cannot be
// renamed or deleted from the TUI.
const defaultBackupGroup = "default"

func (t *backupTab) enterDeleteGroupConfirm() (Tab, tea.Cmd) {
	group := t.realGroup()
	if group == "" {
		return t, warnToast("no group to delete")
	}
	if group == defaultBackupGroup {
		return t, warnToast("default group cannot be deleted")
	}
	t.mode = backupModeDeleteGroupConfirm
	return t, nil
}

// enterRenameMode renames whatever the focused pane shows: the group (left) or
// the text key (right).
func (t *backupTab) enterRenameMode() (Tab, tea.Cmd) {
	if t.focusLeft {
		if row, ok := t.currentGroupRow(); ok && row.isAll {
...
func (t *backupTab) enterImportMode() (Tab, tea.Cmd) {
	group := t.realGroup() // All 视图回落 default
	if group == "" {
		group = defaultBackupGroup
	}
	f := newForm("import backup block from file",
...
func (t *backupTab) enterFilterMode() (Tab, tea.Cmd) {
	t.mode = backupModeFilter
...
// editCurrent opens the selected backup block in vim via tea.ExecProcess.
func (t *backupTab) editCurrent() (Tab, tea.Cmd) {
	it, ok := t.currentItem()
	if !ok {
...
func (t *backupTab) editKey(group, key string) (Tab, tea.Cmd) {
	mgr := t.mgr.Backup
	if mgr == nil {
		return t, func() tea.Msg { return errMsg{err: fmt.Errorf("backup manager unavailable")} }
	}
...
func (t *backupTab) finishAfterEdit(es *backup.EditorSession, runErr error) tea.Msg {
	if runErr != nil {
		// Editor failed/absent: clean up the temp file, do not persist.
		_ = os.Remove(es.TmpPath)
		recordAudit(t.mgr, session.AuditOpBackup, backupTarget(es.Group, es.Key), false, "edit editor failed")
		return errMsg{err: fmt.Errorf("editor failed: %w", runErr)}
	}
	if _, ferr := t.mgr.Backup.FinishEditor(es); ferr != nil {
		recordAudit(t.mgr, session.AuditOpBackup, backupTarget(es.Group, es.Key), false, "edit failed")
		return errMsg{err: ferr}
	}
	recordAudit(t.mgr, session.AuditOpBackup, backupTarget(es.Group, es.Key), true, "edit")
	return backupReloadMsg{}
}

// --- manager operations ---

func (t *backupTab) doDelete(group, key string) tea.Cmd {
	mgr := t.mgr.Backup
	mgrs := t.mgr
	return func() tea.Msg {
		if err := mgr.Delete(group, key); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, key), false, "delete failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, key), true, "delete")
		return backupReloadMsg{}
	}
}

// doRenameKey atomically renames a backup block (content untouched).
func (t *backupTab) doRenameKey(group, oldKey, newKey string) tea.Cmd {
	if oldKey == newKey {
	...
	return func() tea.Msg {
		if err := mgr.RenameKey(group, oldKey, newKey); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, oldKey), false, "rename failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, newKey), true, "rename "+oldKey)
		return renameDoneMsg{group: group, key: newKey, text: "renamed to " + newKey}
	}
}

// doRenameGroup renames a backup group and keeps every block inside it.
func (t *backupTab) doRenameGroup(oldName, newName string) tea.Cmd {
	if oldName == newName {
	...
	return func() tea.Msg {
		if err := mgr.RenameGroup(oldName, newName); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, "text:group:"+oldName, false, "rename group failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, "text:group:"+newName, true, "rename group "+oldName)
		return backupReloadMsg{}
	}
}

// doDeleteGroup deletes a backup group and all of its blocks.
func (t *backupTab) doDeleteGroup(name string) tea.Cmd {
	mgr := t.mgr.Backup
	mgrs := t.mgr
	return func() tea.Msg {
		if err := mgr.DeleteGroup(name); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, "text:group:"+name, false, "delete group failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, "text:group:"+name, true, "delete group")
		return backupReloadMsg{}
	}
}

// doImport encrypts a local file into a backup block (SetFromFile); the source
// file itself is left untouched.
func (t *backupTab) doImport(group, key, path, description string) tea.Cmd {
	if path == "" {
	...
	return func() tea.Msg {
		...
		if err := mgr.SetFromFileWithDescription(group, key, path, &desc); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, key), false, "import failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, backupTarget(group, key), true, "import "+path)
		return renameDoneMsg{group: group, key: key, text: "imported " + key}
	}
}

func (t *backupTab) doCopy() tea.Cmd {
	it, ok := t.currentItem()
	if !ok {
...
func (t *backupTab) doExport(group, key, path string) tea.Cmd {
	mgr := t.mgr.Backup
	return func() tea.Msg {
...
func (t *backupTab) doAddGroup(name, description string) tea.Cmd {
	mgr := t.mgr.Backup
	mgrs := t.mgr
	return func() tea.Msg {
		if err := mgr.AddGroup(name, description); err != nil {
			recordAudit(mgrs, session.AuditOpBackup, "text:group:"+name, false, "add group failed")
			return errMsg{err: err}
		}
		recordAudit(mgrs, session.AuditOpBackup, "text:group:"+name, true, "add group")
		return backupReloadMsg{}
	}
}
...
func (t *backupTab) SetSize(w, h int) { t.width, t.height = w, h }

func (t *backupTab) View() string {
	overlay := ""
	if t.form != nil {
		overlay = t.form.View()
	} else if t.mode != backupModeNormal {
...
func (t *backupTab) viewBaseAt(h int) string {
	leftW := t.width / 4
	if leftW > 26 {
		leftW = 26
	}
...
func (t *backupTab) renderGroups(width, height int) string {
	if !t.loaded {
...
func (t *backupTab) renderItems(width, height int) string {
	if !t.loaded {
		return emptyStateStyle.Render("loading backup blocks…")
	}
	group := t.currentGroup()
	if group == "" {
	...
	if len(items) == 0 {
		hint := "no backup blocks in this group"
		if t.filterBox.Term() != "" {
...
func (t *backupTab) renderModal() string {
	switch t.mode {
	case backupModeBatchDeleteConfirm:
		targets := t.selectedTargets()
		...
		return modalBox(t.width, t.height, fmt.Sprintf("delete %d backup blocks?", len(targets)),
	...
	case backupModeDeleteGroupConfirm:
		group := t.currentGroup()
		body := fmt.Sprintf("group %s and all its backup blocks will be deleted.", group)
		return modalBox(t.width, t.height, "delete group "+group+"?", body, "enter/y confirm · esc/n cancel")
	case backupModeDeleteConfirm:
		it, _ := t.currentItem()
	...
	case backupModeNewKey:
		return modalBox(t.width, t.height, "new backup block — key or group:key", t.input.View(), "enter open vim · esc cancel")
	case backupModeAddGroup:
...
// Compile-time guard: *backupTab satisfies Tab.
var _ Tab = (*backupTab)(nil)
