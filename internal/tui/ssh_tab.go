...
func (t *sshTab) View() string {
	...
	for _, host := range t.hosts {
		hostLines = append(hostLines, fmt.Sprintf("%s → %s", host.Alias, host.Hostname))
	}
	keyLines := make([]string, 0, len(t.keyPairs))
	for _, key := range t.keyPairs {
		label := key.Fingerprint
		if label == "" {
			label = "pubkey: none"
		}
		keyLines = append(keyLines, fmt.Sprintf("%s · %s", key.Name, label))
	}
	half := t.width / 2
	if half < 20 {
		half = max(t.width/2, 1)
	}
	hosts := windowedPane("Hosts", hostLines, t.hostIndex, t.height, half)
	keys := windowedPane("KeyPairs · private keys masked", keyLines, t.keyIndex, t.height, t.width-half)
	return lipgloss.JoinHorizontal(lipgloss.Top, hosts, strings.Repeat(" ", 2), keys)
}
