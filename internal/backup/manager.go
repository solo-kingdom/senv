...
// Manager handles backup block operations
type Manager struct {
...
// NewManager creates a new backup manager with password
...
// NewManagerWithKey creates a new backup manager with a derived key
...
// saveBackupFile saves a backup entry using key or password
func (m *Manager) saveBackupFile(group, key string, entry *storage.BackupEntry) error {
	if m.key != nil {
		return m.storage.SaveBackupFileWithKey(group, key, entry, m.key)
	}
	return m.storage.SaveBackupFile(group, key, entry, m.password)
}

// loadBackupFile loads a backup entry using key or password
func (m *Manager) loadBackupFile(group, key string) (*storage.BackupEntry, error) {
	if m.key != nil {
		return m.storage.LoadBackupFileWithKey(group, key, m.key)
	}
	return m.storage.LoadBackupFile(group, key, m.password)
}
...
func (m *Manager) requireGroup(group string) error {
	exists, err := m.storage.BackupGroupExists(group)
	if err != nil {
		return err
	}
	if !exists {
		return storage.ErrGroupMissing("backup", group)
	}
	return nil
}

// Set sets a backup entry in a group. The group must already exist.
func (m *Manager) Set(group, key, value string) error {
	return m.SetWithDescription(group, key, value, nil)
}

// SetWithDescription sets a backup value. description nil keeps the existing
...
func (m *Manager) SetWithDescription(group, key, value string, description *string) error {
	...
	if len(value) > storage.MaxBackupSize {
		return fmt.Errorf("backup value exceeds %d bytes limit (%d bytes)", storage.MaxBackupSize, len(value))
	}
	if err := m.requireGroup(group); err != nil {
		return err
	}

	entry, err := m.loadBackupFile(group, key)
	if err != nil {
		entry = storage.NewBackupEntry(value)
	} else {
		entry.Value = value
	...
	return m.saveBackupFile(group, key, entry)
}

// Get retrieves a backup entry's value from a group
...
// Delete deletes a backup entry from a group
func (m *Manager) Delete(group, key string) error {
	...
	_, err := m.loadBackupFile(group, key)
	if err != nil {
		return err
	}

	return m.storage.DeleteBackupFile(group, key)
}

// BackupInfo contains metadata about a backup entry for listing
...
// List lists all backup entries in a group with metadata
// List 列出分组内 text 条目，附耗时日志。
func (m *Manager) List(group string) ([]BackupInfo, error) {
	st := perflog.Start("backup.list").With("group", group)
...
func (m *Manager) listEntries(group string) ([]BackupInfo, error) {
	if err := validateGroup(group); err != nil {
		return nil, err
	}
	keys, err := m.storage.ListBackupFiles(group)
	if err != nil {
		return nil, err
	}
	...
	for _, key := range keys {
		entry, err := m.loadBackupFile(group, key)
		if err != nil {
			return nil, fmt.Errorf("failed to load backup %q in group %q: %w", key, group, err)
		}
...
// SetFromFile sets a backup entry from a file
...
// SetFromReader sets a backup entry from an io.Reader
...
func (m *Manager) PrepareEditor(group, key string) (*EditorSession, error) {
	...
	var original string
	if entry, err := m.loadBackupFile(group, key); err == nil {
		original = entry.Value
	}

	tmpFile, err := os.CreateTemp("", "senv-backup-*.tmp")
	if err != nil {
...
func (m *Manager) FinishEditor(s *EditorSession) (bool, error) {
	...
	defer func() { _ = os.Remove(s.TmpPath) }()
...
// SetViaEditor opens an editor for creating or editing a backup entry. It is the
...
// GetToFile writes a backup entry using the private default export mode.
...
// GetToFileWithMode writes a backup entry using a mode selected for this
// operation only.
...
// GetToClipboard copies a backup entry's value to the system clipboard
...
// AddGroup creates a new backup group. Description is required.
func (m *Manager) AddGroup(name string, description string) error {
	...
	groups, err := m.storage.ListBackupGroups()
	if err != nil {
	...
	if err := m.storage.AddBackupGroup(name); err != nil {
	...
	if err := m.storage.SaveBackupGroupMetaWithKey(name, meta, cryptoKey); err != nil {
		_ = m.storage.DeleteBackupGroup(name)
...
func (m *Manager) EnsureGroup(name, description string) error {
	...
	exists, err := m.storage.BackupGroupExists(name)
	if err != nil {
		return err
	}
	if exists {
...
// EnsureDefault creates backup group "default" when missing (existing vaults).
func (m *Manager) EnsureDefault() error {
	return m.EnsureGroup("default", "Default backup group")
}

// DeleteGroup deletes a backup group and all its contents
func (m *Manager) DeleteGroup(name string) error {
	...
	if err := m.storage.DeleteBackupGroup(name); err != nil {
		return fmt.Errorf("failed to delete group %s: %w", name, err)
	}
	return nil
}

// ListGroups lists all backup groups with their key counts
...
func (m *Manager) ListGroups() ([]GroupInfo, error) {
	st := perflog.Start("backup.list-groups")
...
func (m *Manager) listGroupsInfo() ([]GroupInfo, error) {
	...
	for _, name := range groups {
		keys, err := m.storage.ListBackupFiles(name)
		if err != nil {
			return nil, fmt.Errorf("failed to list backup group %q: %w", name, err)
		}
		desc := ""
		if cryptoKey, err := m.resolveCryptoKey(); err == nil {
			if meta, err := m.storage.LoadBackupGroupMetaWithKey(name, cryptoKey); err == nil && meta != nil {
...
type Snapshot struct {
	...
	Items  map[string][]BackupInfo // group → 条目元数据（与 List 返回逐字段一致）
}
...
func (m *Manager) Snapshot() (*Snapshot, error) {
	st := perflog.Start("backup.snapshot")
	vault, err := m.loadBackupVault()
	if err != nil {
		st.End(false)
		return nil, err
	}
	out := &Snapshot{
		...
		Items:  make(map[string][]BackupInfo, len(vault.Groups)),
	}
	total := 0
	for _, name := range vault.Groups {
		...
		for _, f := range entries {
			infos = append(infos, BackupInfo{Key: f.Key, Description: f.Entry.Description, Size: f.Entry.Size, UpdatedAt: f.Entry.UpdatedAt})
		}
...
// loadBackupVault 是 Snapshot 的装载缝：key 与 password 两种认证同等待遇。
func (m *Manager) loadBackupVault() (*storage.BackupVaultSnapshot, error) {
	if m.key != nil {
		return m.storage.LoadBackupVaultWithKey(m.key)
	}
	return m.storage.LoadBackupVault(m.password)
}
...
// RenameKey atomically renames a backup block inside its group. Contents, size
...
func (m *Manager) RenameKey(group, oldKey, newKey string) error {
	...
	if _, err := m.loadBackupFile(group, oldKey); err != nil {
		return fmt.Errorf("backup block %s/%s not found: %w", group, oldKey, err)
	}
	if _, err := m.loadBackupFile(group, newKey); err == nil {
		return fmt.Errorf("backup block %s already exists in group %s", newKey, group)
	}
	return m.storage.RenameBackupFile(group, oldKey, newKey)
}

// RenameGroup renames a backup group directory; every block inside keeps its
// content.
func (m *Manager) RenameGroup(oldName, newName string) error {
	...
	meta, err := m.storage.LoadBackupGroupMetaWithKey(oldName, cryptoKey)
	if err != nil {
		return err
	}
	if err := m.storage.RenameBackupGroup(oldName, newName); err != nil {
		return err
	}
	if meta != nil {
		meta.Name = newName
		if err := m.storage.SaveBackupGroupMetaWithKey(newName, meta, cryptoKey); err != nil {
			_ = m.storage.RenameBackupGroup(newName, oldName)
