...
// setupTestTextManager creates a temporary backup manager for testing
func setupTestTextManager(t *testing.T) (*Manager, string) {
	t.Helper()

	tmpDir, err := os.MkdirTemp("", "senv-backup-test-*")
	if err != nil {
...
func mustSet(t *testing.T, mgr *Manager, group, key, value string) {
	t.Helper()
...
func TestSetOverwrite(t *testing.T) {
	...
	if err := mgr.Set("notes", "KEY", "original"); err != nil {
		t.Fatal(err)
	}
	if err := mgr.Set("notes", "KEY", "updated"); err != nil {
		t.Fatal(err)
	}
...
func TestGetNotFound(t *testing.T) {
	...
	if err == nil {
		t.Error("Should error when getting nonexistent backup")
	}
}
...
func TestSetExceedsSizeLimit(t *testing.T) {
	...
	largeValue := strings.Repeat("x", storage.MaxBackupSize+1)
...
func TestSetMissingGroup(t *testing.T) {
	mgr, _ := setupTestTextManager(t)
	err := mgr.Set("nope", "KEY", "val")
	if err == nil {
...
func TestEnsureDefaultIdempotent(t *testing.T) {
	...
	if err := mgr.Set("default", "DUMP", "x"); err != nil {
		t.Fatal(err)
	}
	got, err := mgr.Get("default", "DUMP")
	if err != nil || got != "x" {
		t.Fatalf("got %q err %v", got, err)
	}
}

func TestEnsureDefaultRecreatesMissing(t *testing.T) {
	mgr, _ := setupTestTextManager(t)
	if err := mgr.DeleteGroup("default"); err != nil {
		t.Fatal(err)
	}
	if err := mgr.Set("default", "DUMP", "x"); err == nil {
		t.Fatal("set into deleted default should fail before EnsureDefault")
	}
	if err := mgr.EnsureDefault(); err != nil {
		t.Fatal(err)
	}
	if err := mgr.Set("default", "DUMP", "x"); err != nil {
		t.Fatalf("set after EnsureDefault: %v", err)
	}
}
...
func TestSetAtSizeLimit(t *testing.T) {
	...
	exactValue := strings.Repeat("x", storage.MaxBackupSize)
	...
	if len(value) != storage.MaxBackupSize {
		t.Errorf("Expected size %d, got %d", storage.MaxBackupSize, len(value))
	}
}
...
func TestDelete(t *testing.T) {
	...
	if err := mgr.Set("notes", "TEMP", "to be deleted"); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteNotFound(t *testing.T) {
	...
	if err == nil {
		t.Error("Should error when deleting nonexistent backup")
	}
}
...
func TestList(t *testing.T) {
	...
	if err := mgr.Set("notes", "README", "content 1"); err != nil {
		t.Fatal(err)
	}
	if err := mgr.Set("notes", "TODO", "content 2"); err != nil {
		t.Fatal(err)
	}
	if err := mgr.Set("notes", "NOTES", "content 3"); err != nil {
		t.Fatal(err)
	}
...
func TestAddGroup(t *testing.T) {
	...
	// Verify by saving a backup to the new group
...
func TestAddGroupDuplicate(t *testing.T) {
	...
	if err := mgr.AddGroup("test-group", "test"); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteGroup(t *testing.T) {
	...
	mustSet(t, mgr, "test-group", "KEY1", "value1")
	mustSet(t, mgr, "test-group", "KEY2", "value2")
...
func TestListGroups(t *testing.T) {
	...
	mustSet(t, mgr, "notes", "KEY1", "v1")
	mustSet(t, mgr, "keys", "KEY2", "v2")
	mustSet(t, mgr, "templates", "KEY3", "v3")
...
func TestGetToFile(t *testing.T) {
	...
	mustSet(t, mgr, "notes", "README", "file output content")
