...
MCP Tab SHALL 提供档案写操作：`n` 新建、`e` 编辑选中档案、`d` 删除。新建表单 SHALL 收集别名、command（必填）、args、env、description；transport MUST 固定为 `stdio` 且不出现在表单中。编辑表单 SHALL 使别名只读，MUST NOT 提供重命名键。args SHALL 以一行一个参数经 `$EDITOR` 编辑并保序；env SHALL 以一行 `KEY=VALUE` 经 `$EDITOR` 编辑，值按模板原样存储。删除 SHALL 二次确认；若本机台账显示该档案已导出，确认文案 SHALL 列出相关 agent id，并说明删除不撤回已导出条目。写操作 SHALL 调用既有档案管理接口，成功后刷新左栏；失败 SHALL 经统一提示条回显且不改变既有档案。
...
MCP Tab 的列表、详情弹层、计划页与 toast MUST NOT 渲染 env 字面量；列表与详情 MAY 展示 env 键名与引用模板原文。用户在档案表单中显式打开 `$EDITOR` 编辑 env 字段时，编辑器是 TUI 内唯一允许出现字面量的解密面；主表单视图 MUST 只显示键名或键数量。
...
- **WHEN** 导出计划含将写入明文 env 的条目
- **THEN** 计划页标注「明文 env」与目标路径，不显示解析后的值
