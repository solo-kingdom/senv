...
#### Scenario: SSH 资产 kind 被接受

- **WHEN** push 或 pull 条目分别为 `ssh_host`、`ssh_keypair` 且 grp 为空、key 为安全单路径段（alias/name）
- **THEN** 条目继续按正常同步流程处理
...
- **WHEN** `config` 携带 grp、`env_meta` 携带 key、`llm_provider`、`mcp_server`、`ssh_host` 或 `ssh_keypair` 携带 grp 或 key 为空，或要求的 grp/key 为空
- **THEN** 条目在访问文件系统前被拒绝
...
人工添加进 vault 的 LLM Provider 档案、MCP Server 档案与 SSH 资产档案（Host 与 KeyPair，KeyPair 档案含私钥本体）SHALL 作为配置源随同步通道双向分发（push 收集、pull 落盘），档案密文落回本机数据目录的 `llm_providers/`、`mcp_servers/`、`hosts/`、`keypairs/` 收集目录。首次在新机器同步时 SHALL 默认拉取全部配置源档案，不要求显式 opt-in；既有 `--accept-remote` 重建路径保持不变。配置源条目发生冲突时，系统 SHALL 沿用与其他 kind 相同的 revision 乐观锁与冲突解决流程，且 SHALL 在冲突报告与审计输出中额外给出本地与远端的 alias 与 revision 对照提示。SSH 资产条目的冲突报告 SHALL 仅渲染元数据（别名/revision/删除状态/大小），MUST NOT 解码或渲染档案明文——KeyPair 档案的私钥明文 MUST NOT 出现在任何渲染输出中。
...
#### Scenario: 新机器首次同步拉到 SSH 资产档案

- **WHEN** 一台没有任何 SSH 资产的新机器完成首次 server 模式 pull
- **THEN** 远端 vault 中的全部 `ssh_host` / `ssh_keypair` 档案密文落回本机 `hosts/`、`keypairs/` 目录，`senv ssh host list` 与 `senv keypair list` 可见

#### Scenario: 配置源档案本地修改后推送

- **WHEN** 用户在本机 `senv ai provider add`、`senv mcp add`、`senv ssh host add` 或 `senv keypair import` 一条新档案后执行 push
- **THEN** 该档案以对应 kind 进入待推送批次并成功上传，远端 revision 递增
...
#### Scenario: SSH 档案冲突仅元数据渲染

- **WHEN** 两台机器分别修改同一 KeyPair 档案触发冲突
- **THEN** 冲突报告仅包含别名、两侧 revision/删除状态/大小等元数据与对照提示，不包含档案明文或私钥内容
