...
磁盘逃生舱（显式或因无法提供安全存储而默认）MUST 保持 0600/0700 权限、独占/原子写入与 boot ID 校验。错误信息在 fail closed 时 MUST 给出可行动指引（平台推荐存储与逃生舱说明）。stderr 安全警告（派生会话密钥以明文存储在 0600 磁盘文件中）SHALL 只在初始化时输出：`session start` 写入新派生密钥命中磁盘逃生舱（含显式 `--insecure-cache`），每进程至多一次；已存储密钥的后续读取、滑动续期、显式 renew 与遗留收养 MUST NOT 重复输出该警告，避免每次命令都刷屏。读路径最终选中磁盘逃生舱缓存时——无论它以较新胜过同时可读的安全存储缓存，还是安全存储读取失败后的回退——系统 SHALL 提供进程内状态供支持操作审计的命令在审计中留下「缓存来自磁盘逃生舱」的记录；MUST NOT 在无审计痕迹的情况下静默使用磁盘逃生舱缓存完成解密。
...
#### Scenario: 双缓存时逃生舱胜出留审计痕迹

- **WHEN** 同一 slot 同时读到可读的安全存储缓存与磁盘逃生舱缓存，逃生舱缓存较新被选中
- **THEN** 进程继续以逃生舱缓存完成解密，不输出 stderr 警告，但留下「缓存来自磁盘逃生舱」审计痕迹；安全存储缓存不被删除

#### Scenario: 安全存储读取失败回退逃生舱留审计痕迹

- **WHEN** 安全存储读取失败（锁定/不可用），同 slot 存在可用的磁盘逃生舱缓存并被选中
- **THEN** 命令成功完成，不输出 stderr 警告，但留下「缓存来自磁盘逃生舱」审计痕迹
