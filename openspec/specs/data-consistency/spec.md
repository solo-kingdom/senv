# data-consistency Specification

## Purpose

检测与防止 `metadata.json` 与加密数据文件（`env_*.json.enc`、`texts/`、config）之间的密钥不同步（desync）。通过 init 防呆、`senv doctor` 体检与 git pull 后自检，在 metadata 与数据文件不属于同一套密钥时给出明确诊断与恢复指引，且不泄露明文或 derived key。
