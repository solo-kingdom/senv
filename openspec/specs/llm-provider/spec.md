...
任一步失败 MUST NOT 留下「档案已改名但凭据未迁」或「凭据已迁但档案仍旧名」的可观察中间态。命令输出 SHALL 包含受影响的 agent 指针数量，并提示 coding agent 原生配置仍使用 `senv-<old>` 标识，需重跑 `senv ai switch` 才会写入 `senv-<new>`。rename MUST NOT 改写任何 coding agent 原生配置文件或 catalog 文件。输出与审计 MUST NOT 含凭据明文。TUI SHALL 提供等价操作。
...
- **THEN** 档案改名成功，`credential_ref` 仍为 `env:llm/KEY`，`env:llm/KEY` 条目保留，不创建或删除 `llm-keys/*`

#### Scenario: 目标别名冲突
- **WHEN** 目标别名已存在
- **THEN** 命令非 0 退出，原档案、凭据与指针全部不变
...
- **THEN** 档案改名成功，`credential_ref` 变为 `text:llm-keys/acme-prod`，不因凭据缺失而失败
