...
**Reason**: 「V1 仅接受 `stdio`」的传输规则被多传输（`stdio` / `http` / `sse`）校验取代，其中「非 stdio 传输拒绝」场景不再成立。
**Migration**: 由本 change 新增的「档案传输与字段校验」取代；stdio 语义（command 必填、args/env 可选）与别名冲突行为不变。
